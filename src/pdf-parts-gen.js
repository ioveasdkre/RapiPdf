...
export function getInfoDef(spec, bookTitle, localize) {
  ...
  if (spec.info) {
    ...
    content = [
      ...
      spec.info.title
        ? { text: spec.info.title, style: ['title', 'right', 'chinese'] }
        : '',
      spec.info.version
        ? {
...
export function getSecurityDef(spec, localize) {
  ...
  if (spec.securitySchemes) {
    ...
    const tableContent = [
      [
        { text: localize.key, style: ['small', 'b', 'header'] },
  ...
  const tableContent = [
    [
      ...
      //   { text: includeExample ? localize.example : '', style: ['sub', 'b', 'alternate', 'header'] },
      {
        ...
        style: ['small', 'b', 'alternate', 'header'],
      },
    ],
  ];
...
function getResponseDef(
  ...
  for (const statusCode in responses) {
    ...
    respDef.push({
      text: [
        {
          text: responses[statusCode].description,
          style: ['small', 'chinese'],
        },
      ],
      //   margin: [0, 10, 0, 0],
  ...
  spec.tags.map((tag) => {
    ...
    for (let j = 0; j < tag.paths.length; j++) {
      ...
      const pathSummaryMarkDef = {
        stack: markdownToPdfmake(
          `${path.method} ${path.path} ${
            path.security
              ? ' + '.concat(path.security.map((x) => Object.keys(x)[0]).join())
              : ''
          }`
        ),
        style: ['primary'],
